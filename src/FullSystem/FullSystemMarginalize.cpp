...
void FullSystem::flagFramesForMarginalization(FrameHessian *newFH) {
    ...
    for (int i = 0; i < (int)frameHessians.size(); i++) {
        ...
        Vec2 refToFh =
            AffLight::fromToVecExposure(frameHessians.back()->ab_exposure, fh->ab_exposure, frameHessians.back()->aff_g2l(), fh->aff_g2l());
        ...
        if ((in < setting_minPointsRemaining * (in + out) || fabs(logf((float)refToFh[0])) > setting_maxLogAffFacInWindow) &&
            ...
            //			printf("MARGINALIZE frame %d, as only %'d/%'d points remaining (%'d %'d %'d %'d). VisInLast %'d / %'d. traces %d,
            // activated %d!\n", 					fh->frameID, in, in+out, 					(int)fh->pointHessians.size(),
            // (int)fh->immaturePoints.size(), 					(int)fh->pointHessiansMarginalized.size(), (int)fh->pointHessiansOut.size(), 					visInLast,
            //outInLast, 					fh->statistics_tracesCreatedForThisFrame, fh->statistics_pointsActivatedForThisFrame);
            fh->flaggedForMarginalization = true;
            flagged++;
        } else {
            //			printf("May Keep frame %d, as %'d/%'d points remaining (%'d %'d %'d %'d). VisInLast %'d / %'d. traces %d, activated
            //%d!\n", 					fh->frameID, in, in+out, 					(int)fh->pointHessians.size(),
            //(int)fh->immaturePoints.size(), 					(int)fh->pointHessiansMarginalized.size(), (int)fh->pointHessiansOut.size(), 					visInLast,
            //outInLast, 					fh->statistics_tracesCreatedForThisFrame, fh->statistics_pointsActivatedForThisFrame);
        }
