...
void EnergyFunctional::setAdjointsF(CalibHessian *Hcalib) {
    ...
    for (int h = 0; h < nFrames; h++)
        for (int t = 0; t < nFrames; t++) {
            ...
            Vec2f affLL =
                AffLight::fromToVecExposure(host->ab_exposure, target->ab_exposure, host->aff_g2l_0(), target->aff_g2l_0()).cast<float>();
            AT(6, 6) = -affLL[0];
            AH(6, 6) = affLL[0];
            AT(7, 7) = -1;
            AH(7, 7) = affLL[0];
    ...
    for (int i = min; i < max; i++) {
        ...
        for (EFResidual *r : p->residualsAll) {
            ...
            for (int i = ((patternNum >> 2) << 2); i < patternNum; i++) {
                float Jdelta =
                    rJ->JIdx[0][i] * Jp_delta_x_1 + rJ->JIdx[1][i] * Jp_delta_y_1 + rJ->JabF[0][i] * dp[6] + rJ->JabF[1][i] * dp[7];
